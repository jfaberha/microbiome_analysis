...
# After running all pairwise comparisons get list of any OTU that shows differential abundance in any Wald test
# Then extract results for all differentially abundant OTUs for each comparision. These results were used to build table.
sigtab2 <- res[c("OTU_1", "OTU_10", "OTU_105", "OTU_107", "OTU_117", "OTU_15", "OTU_156", "OTU_18", "OTU_197", "OTU_2", "OTU_20", "OTU_21", "OTU_212", "OTU_25", "OTU_26", "OTU_33", "OTU_39", "OTU_4", "OTU_48", "OTU_5", "OTU_6", "OTU_79", "OTU_9"),]
sigtab2 = cbind(as(sigtab2, "data.frame"), as(tax_table(phyloseq_data_ds)[rownames(sigtab2), ], "matrix"))
...
####################################################################
#Define new subsets here for exploratory purposes
#You probably just want to analyze the full dataset either raw or normalized for most analyses
#The "check" variable specifically is there to filter out samples with NA's for all variables of interest (most useful for PERMANOVA and ANOVA analyses)
...
microB_subset <- noNA # rarified dataset, no NAs in all variables of interest
microB_subset <- noNA_norarify  # unrarified dataset, no NAs in all variables of interest
microB_subset <- noNA_ds2 # VST normalized dataset, floored at 0, no NAs in all variables of interest
microB_subset <- phyloseq_data # rarified dataset
microB_subset <- phyloseq_ds # VST normalized dataset
microB_subset <- phyloseq_data_norarify # unrarified dataset
microB_subset <- phyloseq_data_ds2 # VST normalized dataset, floored at 0
...
plot_ordination(microB_subset, ordu5, type = "scree")
stressplot(ordu)

#Final beta diversity plots
plot_ordination(microB_subset, ordu2, color="treat_c", title=paste("UniFrac NMDS")) + geom_point(size=4) + coord_fixed()
plot_ordination(microB_subset, ordu3, color="treat_c", title=paste("WuniFrac NMDS")) + geom_point(size=4) + coord_fixed()
...
plot_ordination(microB_subset, ordu2, color="treat_c", shape = "stock",title=paste("UniFrac NMDS")) + geom_point(size=4) + coord_fixed()
plot_ordination(microB_subset, ordu3, color="treat_c", shape = "stock",title=paste("WuniFrac NMDS")) + geom_point(size=4) + coord_fixed()
...
plot_ordination(microB_subset, ordu3, color="gen", shape = "stock",title=paste("WuniFrac NMDS")) + geom_point(size=4) + coord_fixed()
...
#PERMANOVA by adonis is recommended instead, but this analysis can be used for data exploration.
...
adonis_res
adonis_res <- adonis2(formula = phydist ~ metaD$treat_c * metaD$treat_tank_orig, data = metaD) 
adonis_res
adonis_res <- adonis2(formula = phydist ~ metaD$treat_c * metaD$source_tank, data = metaD) 
adonis_res
...
adonis_res
adonis_res <- adonis2(formula = phydist ~ metaD$student, data = metaD) 
adonis_res
...
#This is a major assumption of the PERMANOVA test, therefore if specific 
#variable is significant for this test the assumption is violated.
